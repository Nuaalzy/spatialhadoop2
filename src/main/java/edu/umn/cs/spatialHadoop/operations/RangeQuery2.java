 ...
 *
 */
@OperationMetadata(shortName = "rangequery2",
description = "Finds all objects that ovelap a polygon query range")
public class RangeQuery2 {
        ...
        for (int i = i1; i < i2; i++) {
          try {
            ...
            while (reader.nextKeyValue()) {
              ...
              for (Shape s : shapes) {
                Point p = (Point) s;
                if ( queryRange.contains(p.x,p.y) ) {
                  results++;
                  if (output != null)
                    output.collect((S) s);
                }
  ...
  public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
    ...
    // 判断查询范围是否存在
    if (params.get("rect") == null && params.get("polygon") == null) {
    ...
    //获取查询范围
    final Polygon[] queryRanges = params.getShapes("polygon", new Polygon());
    ...
    for (int i = 0; i < queryRanges.length; i++) {
      ...
      OperationsParams.setShape(queryParams, "polygon", queryRanges[i]);
      if (OperationsParams.isLocal(new JobConf(queryParams), inPath)) {
        // Run in local mode
        final Polygon queryRange = queryRanges[i];
